#soal no 3

#a
x = 6
lambda = 4.5
dpois(x,lambda)

#b
set.seed(0)
babies <- data.frame('data' = rpois(365, 4.5))
#babies

babies %>% ggplot() +
  geom_histogram(aes(x = data,
                     y = stat(count / sum(count)),
                     fill = data == 6),
                 binwidth = 1,
                 color = 'black',) +
  scale_x_continuous(breaks = 0:10) + 
  labs(x = 'Number of babies born per period',
       y = 'Proportion',
       title = '365 simulated births in a hospital with Pois(lambda = 4.5)') +
  theme_bw()

#c
set.seed(0)
babies <- data.frame('data' = rpois(365, 4.5))
babies %>% dplyr::summarize(six_babies = sum(babies$data == 6) / n())
#dapat dilihat bahwa hasil simulasi nya sekitar 12.3%
#(berdasarkan set.seed(0) dan perhitungan di atas) 
#sedangkan nilai exact nya yaitu 12,8%
#sehingga dapat disimpulkan nilai simulasi mendekati nilai eksak

#d
#ratarata
lambda = 4.5
lambda

#variansi
lambda = 4.5
lambda

